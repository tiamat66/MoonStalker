...
import si.vajnartech.moonstalker.androidsvg.SVG.Style;
...
public class SVGParser
{
    ...
    for (int i = 0; i < attributes.getLength(); i++) {
      if (SVGAttr.fromString(attributes.getLocalName(i)) == SVGAttr.points) {
        ...
        while (!scan.empty()) {
          ...
          if (Float.isNaN(y))
            throw new SVGParseException(
                "Invalid <" + tag + "> points attribute. There should be an even number of coordinates.");
          scan.skipCommaWhitespace();
          points.add(x);
          points.add(y);
        }
  ...
  private static void parsePreserveAspectRatio(SVG.SvgPreserveAspectRatioContainer obj,
                                               String val) throws SVGParseException
  {
  ...
  {
    Integer CustomColour(String name);
